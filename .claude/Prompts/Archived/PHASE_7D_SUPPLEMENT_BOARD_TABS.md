# Phase 7D Supplement: Board Type Tabs & Investment Integration

## 🎯 ADDITIONAL REQUIREMENTS
Setelah migration `MIGRATION_MODEL_GROUPS.sql` dijalankan, Phase 7D perlu update untuk mendukung:
...
```tsx
// Add board breakdown section to InvestmentSummary.tsx

interface InvestmentSummaryProps {
  model: SimilarModel;
  showBoardBreakdown?: boolean;  // NEW
}

// Add this section in the component:
...
export async function GET(
  ...
  return NextResponse.json({
    stations: stations.map(s => ({
      ...
      cycleTime: s.station_master?.cycle_time_seconds,
      area: s.area,
    })),
  });
}
...
export async function POST(request: Request) {
  ...
  const similarModels = groups.map(g => ({
    ...
    similarity: 0.85, // Calculate actual similarity
    totalBoards: g.total_boards,
    totalStations: g.total_stations,
    matchedStations: Math.floor(g.total_stations * 0.8), // Calculate actual
...
### Phase 7D Additional:
- [ ] Model groups shown instead of individual models
- [ ] Board type tabs in modal
- [ ] Per-board station tables load correctly
- [ ] Investment breakdown per board type
- [ ] Main Board variants show RAM/eMMC specs
- [ ] Total investment from database (not calculated)
- [ ] API endpoints return grouped data
...
1. **Migration Order**:
   - Run `MIGRATION_MODEL_GROUPS.sql` first
   - Run `MIGRATION_CSV_IMPORT.sql` to populate data
   - Then implement Phase 7D updates

2. **Legacy Compatibility**:
   - `stationCount` and `manpower` kept for backward compatibility
   - Use new fields (`totalStations`, `totalManpower`) in new code

3. **Performance**:
   - Use `v_model_groups_summary` view for list queries
   - Lazy load stations per board to avoid large payloads
